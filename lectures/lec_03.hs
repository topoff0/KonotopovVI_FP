...
--NOTE: Список должен заканчиваться пустым списком

--NOTE: Можно сделать список списков:
(1 : 2 : []) : (3 : 4 : []) : []
...
myReverse xs = localRev xs []
    where localRev :: [a] -> [a] -> [a]
          localRev [] ys     = ys -- localRev это локальная функция (ее нельзя вызвать извне функции myReverse)
